...
### 5. Earn Reputation
Every on-chain action feeds an off-chain reputation engine. Scores (0–100) computed from graduation rates, trader diversity, cumulative graduated volume, account age, and burst/self-trade penalties. Sigmoid formula: `100 / (1 + e^(-8.0 × (raw - 0.4)))` (recalibrated 2026-07-17 — previously `K=0.15` caused blank/low-activity wallets to default to ~48 rep; now blank wallets score ≈3.9). Wallets with too little activity (no tokens created, no graduations) are shown as **Unranked** instead of a misleading number. 12 milestone badges (10 earnable + Founder + First Graduation). Computed client-side per profile view; daily Merkle-root anchoring to an on-chain ProfileRegistry is spec'd for Stage 6 (not yet wired).
