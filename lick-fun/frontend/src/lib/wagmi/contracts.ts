...
import { GAS_LIMITS } from "@/lib/gas-limits";
...
export function useBetYes() {
  ...
  const betYes = async (tokenAddress: `0x${string}`, valueInWei: bigint) => {
    return writeContractAsync({
      ...
      args: [tokenAddress],
      value: valueInWei,
      gas: GAS_LIMITS.CREATE_PREDICTION,
    });
  };
...
export function useClaimWinnings() {
  ...
  const claim = async (tokenAddress: `0x${string}`) => {
    return writeContractAsync({
      ...
      args: [tokenAddress],
      gas: GAS_LIMITS.RESOLVE_PREDICTION,
    });
  };
